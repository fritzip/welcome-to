...
function showP() {
  console.log(
    "{ 'x' : " +
      posx / $("#scoresheet").width() +
      ", 'y' : " +
      posy / $("#scoresheet").height() +
      " },"
  );
}
  ...
  for (var row in houses) {
    for (var i in houses[row]) {
      $("#widget-container").append(
        `<div id='house-${row}-${i}' class='house widget' data-row='${row}' style='left:${
          xscale * houses[row][i].x
        }%;top:${
          yscale * houses[row][i].y
        }%;'><input type="number" min="0" max="17"></div>`
      );
    }
  }
}
...
function initBonus() {
  for (var i in bonus_scores) {
    $("#widget-container").append(
      `<div id='bonus-${i}' class='total-score bonus-score widget' style='left:${
        xscale * bonus_scores[i].x
      }%;top:${yscale * bonus_scores[i].y}%;'><span></span></div>`
    );
  }
}
...
function initParks() {
  ...
  for (var i in parks) {
    ...
    $("#widget-container").append(
      `<div id='park-${i}' class='park widget ${extraClass}' style='left:${
        xscale * parks[i].x
      }%;top:${yscale * parks[i].y}%;' data-row='${parks[i].row}' data-score='${
        parks[i].score
      }'></div>`
    );
    extraClass = "";
  ...
  for (var i in park_scores) {
    $("#widget-container").append(
      `<div id='park-score-${i}' class='total-score park-score widget' style='left:${
        xscale * park_scores[i].x
      }%;top:${yscale * park_scores[i].y}%;'><span></span></div>`
    );
  }
}
...
function initPools() {
  for (var i in pools) {
    $("#widget-container").append(
      `<div id='pool-${i}' class='pool widget checkable' style='left:${
        xscale * pools[i].x
      }%;top:${yscale * pools[i].y}%;'></div>`
    );
  }
  for (var i in pools_checks) {
    $("#widget-container").append(
      `<div id='pool-check-${i}' class='pool-check widget' style='left:${
        xscale * pools_checks[i].x
      }%;top:${yscale * pools_checks[i].y}%;'></div>`
    );
  }
}
...
function initFences() {
  for (var row in fences) {
    for (var i in fences[row]) {
      $("#widget-container").append(
        `<div id='fence-${row}-${i}' class='fence widget' data-row='${row}' style='left:${
          xscale * fences[row][i].x
        }%;top:${yscale * fences[row][i].y}%;'></div>`
      );
    }
  }
}
...
function initWorkSigns() {
  for (var i in worksigns) {
    $("#widget-container").append(
      `<div id='work-${i}' class='work widget checkable' style='left:${
        xscale * worksigns[i].x
      }%;top:${yscale * worksigns[i].y}%;'></div>`
    );
  }
  for (var i in worksigns_radio) {
    $("#widget-container").append(
      `<div id='work-score-${i}' class='work-score widget checkable' data-score='${
        worksigns_radio[i].score
      }' style='left:${xscale * worksigns_radio[i].x}%;top:${
        yscale * worksigns_radio[i].y
      }%;'><span></span></div>`
    );
  }
}
...
function initLandPrices() {
  ...
  for (var i in landprices) {
    ...
    $("#widget-container").append(
      `<div id='landprice-${i}' class='landprice widget ${extraClass}' data-col='${
        landprices[i].col
      }' data-score='${landprices[i].score}' style='left:${
        xscale * landprices[i].x
      }%;top:${yscale * landprices[i].y}%;'></div>`
    );
    extraClass = "";
  }
  for (var i in landcounters) {
    $("#widget-container").append(
      `<div id='landcounter-${i}' class='landcounter widget' style='left:${
        xscale * landcounters[i].x
      }%;top:${yscale * landcounters[i].y}%;'><span></span></div>`
    );
  }
}
...
function updateLandScores() {
  for (var col in landcounters) {
    ...
    if ($(last_checked).length == 0) {
      $("#landprice-score-" + col + ">span").text(
        landprices_score[col] *
          Number($("#landcounter-" + col + ">span").text())
      );
    } else {
      $("#landprice-score-" + col + ">span").text(
        $(last_checked).data("score") *
          Number($("#landcounter-" + col + ">span").text())
      );
    }
  }
...
function initBis() {
  ...
  for (var i in bis) {
    ...
    $("#widget-container").append(
      `<div id='bis-${i}' class='bis widget ${extraClass}' style='left:${
        xscale * bis[i].x
      }%;top:${yscale * bis[i].y}%;'></div>`
    );
    extraClass = "";
  }
}
...
function initCantplay() {
  ...
  for (var i in cantplay) {
    ...
    var res = $("#widget-container").append(
      `<div id='cantplay-${i}' class='cantplay widget ${extraClass}' data-score='${
        cantplay[i].score
      }' style='left:${xscale * cantplay[i].x}%;top:${
        yscale * cantplay[i].y
      }%;'></div>`
    );
    extraClass = "";
...
function initScores() {
  for (var i in total_scores) {
    $("#widget-container").append(
      `<div id='${total_scores[i].id}' class='total-score widget' style='left:${
        xscale * total_scores[i].x
      }%;top:${yscale * total_scores[i].y}%;'><span></span></div>`
    );
  }
}
...
function updateLandCounter() {
  ...
  for (var row = 0; row < 3; ++row) {
    ...
    for (var i = 0; i < housesDOM.length; ++i) {
      ...
      if (
        $(fencesDOM[i]).hasClass("checked") ||
        typeof fencesDOM[i] == "undefined"
      ) {
        if (inHousingEstate && housingEstateSize < 7) {
          housingEstateSize--;
          $("#landcounter-" + housingEstateSize + ">span").text(
            parseInt($("#landcounter-" + housingEstateSize + ">span").text()) +
              1
          );
        }
        housingEstateSize = 0;
...
function renderCards() {
  ...
  $("#action1")
    ...
    .attr("src", "assets/" + actions[i * 3] + ".png");
  $("#action2")
    .addClass("loading-img")
    .attr("src", "assets/" + actions[i * 3 + 1] + ".png");
  $("#action3")
    .addClass("loading-img")
    .attr("src", "assets/" + actions[i * 3 + 2] + ".png");
  ...
  $("#number1")
    .nextAll(":lt(2)")
    .addClass("loading-img")
    .attr("src", "assets/" + actions[i * 3 + 3] + "-ico.png");
  $("#number2")
    .nextAll(":lt(2)")
    .addClass("loading-img")
    .attr("src", "assets/" + actions[i * 3 + 4] + "-ico.png");
  $("#number3")
    .nextAll(":lt(2)")
    .addClass("loading-img")
    .attr("src", "assets/" + actions[i * 3 + 5] + "-ico.png");
}
...
$(document).ready(function () {
  $(
    "#widget-container"
  ).append(`<div id=cityname class='widget' style='left:${xscale * cityname.x}%;top:${yscale * cityname.y}%;'>
  ...
  $(".bonus-check").click(function () {
    ...
    if (result.hasClass("check")) {
      ...
      $("#bonus-" + row + ">span").text(
        bonus_cards_scores[row][number][bonus_id]
      );
    } else {
  ...
  $(".park").click(function () {
    ...
    if (
      ($(this).hasClass("check") &&
        $(last_checked).data("score") === $(this).data("score")) ||
      (!$(this).hasClass("check") &&
        $(first_unchecked).data("score") == $(this).data("score"))
    ) {
      $(this).toggleClass("check");

      $(".park[data-row='" + row + "']").each(function () {
        $(this).removeClass("checkable");
      });
      $(".park[data-row='" + row + "']:not(.check):first").addClass(
        "checkable"
      );
  ...
  $(".landprice").click(function () {
    ...
    var first_unchecked = $(
      ".landprice[data-col='" + col + "']:not(.check):first"
    );
    var last_checked = $(".landprice[data-col='" + col + "'].check:last");
    ...
    if (
      ...
      $(".landprice[data-col='" + col + "']").each(function () {
        $(this).removeClass("checkable");
      });
      $(".landprice[data-col='" + col + "']:not(.check):first").addClass(
        "checkable"
      );
  ...
  $(".bis").click(function () {
    ...
    if (
      ($(this).hasClass("check") &&
        $(last_checked).attr("id") === $(this).attr("id")) ||
      (!$(this).hasClass("check") &&
        $(first_unchecked).attr("id") == $(this).attr("id"))
    ) {
      $(this).toggleClass("check");
      $(".bis").each(function () {
        $(this).removeClass("checkable");
...
$(window).on("load", function () {
  ...
  preloadImages([
    ...
    "assets/bis-ico.png",
    "assets/fence.png",
    "assets/fence-ico.png",
    "assets/landprice.png",
    "assets/landprice-ico.png",
    "assets/park.png",
    "assets/park-ico.png",
    "assets/pool.png",
    "assets/pool-ico.png",
